// Revised entry for MinorHack at April 14th, 2007 at 7:00 pm UTC
// Submitted by Jakub Wasilewski
/// MINORHACK ENTRY by Jakub Wasilewski
/// "POISONED WATER"
/// 55 minutes
/// play with arrows and enter/space, follow instructions on screen
...
void init()
{
	...
	//floodfill(glass, 25, 55, makecol(0, 120, 255));
	for (double f = 0.99; f >= 0.0; f -= 0.05)
	{
		int y = (int)(35.0 + f * 50.0);
		int r1 = (int)(22.0 - f * 6.0);
		int r2 = (int)(13.0 - f * 3.0);
		ellipsefill(glass, 25, y, r1, r2, makecol(0, 120 - (int)(f * 120.0), 255 - (int)(f * 30.0)));
	}
...
class Glass
{
	...
	public:
		...
		Glass(){};
		Glass(int position) :
			nowPos(position), prevPos(position), trans(TRANS_NONE), transF(0.0), rightOne(false)
		{
			for (int i = 0; i < 5; i++)
				newBubble(i);
		};
		...
		void render(BITMAP *bmp)
		{
			...
			for (int i = 0; i < 5; i++)
			{
				...
				circle(bmp, (int)(x+xAdd+bubbles[i].x), (int)(y + bubbles[i].y), 2, makecol(255, 255, 255));
			}
		...
		while(!end)
		{
			while (timer > 0)
			{
				...
				if (state == GAME_CHOOSE)
					...
					{
						int k = readkey() >> 8;
						if ((k == KEY_LEFT) && (arrowPos > 0))
							arrowPos--;
						if ((k == KEY_RIGHT) && (arrowPos < 4))
							arrowPos++;
			...
			clear_bitmap(buf);
			for (int i = 0; i < 5; i++)
				if (((!glasses[i].rightOne) || (state != GAME_SHOW) || (timing % 20 > 10)) && glasses[i].trans == TRANS_UP)
					glasses[i].render(buf);
			for (int i = 0; i < 5; i++)
				if (((!glasses[i].rightOne) || (state != GAME_SHOW) || (timing % 20 > 10)) && glasses[i].trans == TRANS_NONE)
					glasses[i].render(buf);
			for (int i = 0; i < 5; i++)
				if (((!glasses[i].rightOne) || (state != GAME_SHOW) || (timing % 20 > 10)) && glasses[i].trans == TRANS_DOWN)
					glasses[i].render(buf);
			...
			textprintf_centre_ex(buf, font, 320, 20, makecol(255, 170, 0), -1, "ALWAYS PICK THE FLASHING GLASS!");
			textprintf_centre_ex(buf, font, 320, 360, makecol(180, 180, 255), -1, "LEVEL: %d", level);
			if (state == WRONG)
			{
				textprintf_centre_ex(buf, font, 320, 370, makecol(255, 0, 0), -1, "WRONG! Retry (y/n)?");
			}
