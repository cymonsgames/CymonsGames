...
ENEMY enemies[MAX_ENEMIES] = {
  ...
  {"Evil Faerie",5,13,4,5,20,1,0,40,2}
};
...
void newGame() {
    ...
    */
    ...
    player.str = rollDie(races[player.race].min_str,races[player.race].max_str);
...
void updatePlayer() {
    ...
    encounter_occurred = rollDie(1,ENCOUNTER_MAX);

    if (encounter_occurred == ENCOUNTER_RATE) showEncounter();
}
...
void doBattle() {
    ...
    nme_idx = rollDie(1,MAX_ENEMIES-1);

    /* Keep a temporary copy so we can kill it without hurting the prototypes */
    the_enemy = enemies[nme_idx];

    /* Calculate victory condition values in advance */
    ...
    while (the_enemy.hp > 0 && player.is_dead == FALSE && player.hp > 0 && didFlee == FALSE) {
        ...
        switch(key) {
            ...
            case 'a':
                keypad(win,FALSE);
                battleStats(the_enemy,round);
                mvwprintw(win,10,3,"%s attacks the %s with a %s...",player.player_name,the_enemy.enemy_name,weapons[player.weapon].weapon_name);
...
void display_menu(int old_option, int new_option) {
	...
	//mvaddstr(tmarg + MAX_OPTIONS + 2, lmarg - 25,
	//	"Use Up and Down Arrows to select - Enter to Run");
	refresh();
}
