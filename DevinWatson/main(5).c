...
void newGame() {
 ...
 /* JOE: Let's start the player in the middle of the map, shall we?
  * What's this? The player's x,y is based on where they are on the map, not
  * where they are on the screen? Yuppers. */
    ...
    */
    while (grid[player.y][player.x] == WALL || grid[player.y][player.x] == OUTERWALL) {
        player.x = (int)rand()% MAP_WIDTH / 2;
        player.y = (int)rand()% MAP_HEIGHT / 2;
    }
...
void initMap() {
	...
	for(yi=0; yi<MAP_HEIGHT; yi++)
		grid[yi][0] = grid[yi][size_x-1] = OUTERWALL;
	for(xi=0; xi<MAP_WIDTH; xi++)
		grid[0][xi] = grid[size_y-1][xi] = OUTERWALL;
...
/* JOE: Should this maybe be a <= instead of an ==. As an ==, no matter what
 * value you have in ENCOUNTER_RATE you're basiclly looking at a 1 in 100 chance
 * of an encounter. I'll leave that for your to change.
 */
...
void getPlayerInput() {
    ...
    if (player.x > MAP_WIDTH) player.y = MAP_WIDTH;
    /*
        If the tile on the map at the new player coordinates
        is one of the impassible types, we reset the player.
    */
    if (grid[player.y][player.x] == WALL || grid[player.y][player.x] == OUTERWALL) {
        player.x = oldX;
        player.y = oldY;
...
void drawMap(int centerY, int centerX) {
    ...
    if (mapTopX + MAP_DISPLAY_WIDTH > MAP_WIDTH) mapTopX = MAP_WIDTH - MAP_DISPLAY_WIDTH;
    if (mapTopY + MAP_DISPLAY_HEIGHT > MAP_HEIGHT) mapTopY = MAP_HEIGHT - MAP_DISPLAY_HEIGHT;
    ...
    for (YIndex = 0; YIndex < MAP_DISPLAY_HEIGHT; YIndex++) {
        for (XIndex = 0; XIndex < MAP_DISPLAY_WIDTH; XIndex++) {
            if ((YIndex+mapTopY == player.y) && (XIndex+mapTopX == player.x)) {
                ...
                mvwprintw(win,YIndex+1,XIndex+1,player.avatar);
                wattroff(win,COLOR_PAIR(10)| A_BOLD);
            } else if (YIndex + mapTopY > MAP_HEIGHT || YIndex + mapTopY < 0
              || XIndex + mapTopX > MAP_WIDTH || XIndex + mapTopX < 0) {
                /* over the edge (should not happen with current config) */
                wattrset(win,COLOR_PAIR(4));
                mvwaddrawch(win,YIndex+1,XIndex+1,ACS_BLOCK);
                ...
                switch (grid[YIndex+mapTopY][XIndex+mapTopX]) {
                    ...
                    case GRAVEL:
                        ...
                        wattroff(win,COLOR_PAIR(11) | A_DIM);
                        break;
                    case OUTERWALL:
                        wattrset(win,COLOR_PAIR(12) | A_DIM);
                        mvwaddrawch(win,YIndex+1,XIndex+1,ACS_CKBOARD);
                        wattroff(win,COLOR_PAIR(12) | A_DIM);
                        break;
                }
            }
        }
