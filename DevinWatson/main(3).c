...
#define HEAL_STEPS 5
#define MAP_WIDTH 800           //Width in columns
...
void getPlayerInput() {
    ...
    switch(key) {
        ...
        case KEY_UP:
            ...
            break;
        case KEY_DOWN:
            if (player.y < MAP_DISPLAY_HEIGHT - 2) {
                player.y++;
            } else {
                mapTopY++;
            }
            break;
        case KEY_RIGHT:
            if (player.x < MAP_DISPLAY_WIDTH - 2) {
                player.x++;
            } else {
                mapTopX++;
            }
    ...
    if (grid[player.y + mapTopY - 1][player.x + mapTopX - 1] == WALL) {
        player.x = oldX;
        player.y = oldY;
    }
    ...
    if (player.x + mapTopX == MAP_WIDTH) {
        mapTopX = MAP_WIDTH - MAP_DISPLAY_WIDTH;
    }

    //if ( (mapTopX + MAP_DISPLAY_WIDTH) == MAP_WIDTH) mapTopX = 0;
    if (player.y + mapTopY == MAP_HEIGHT - 1) {
        mapTopY = MAP_HEIGHT - MAP_DISPLAY_HEIGHT;
    }
}
